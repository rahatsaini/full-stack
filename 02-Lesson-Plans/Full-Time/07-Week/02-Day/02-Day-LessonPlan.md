...
* Open `18-Stu_Hooks/Solved/models/User.js` in your IDE and explain the following: 
  ...
  * 🔑 We can repeat the preceding steps to create the `beforeUpdate()` method. We then add a conditional statement that will only hash a user's password if it is included in the payload. Without this conditional statement, the user's information could become corrupted and they would not be able to login. Note that this isn't the full implementation for updating a hashed password, but that we'll dive deeper into that in a later module:
